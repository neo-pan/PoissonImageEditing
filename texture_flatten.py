...
def edge_detection(
    image: np.ndarray, method: str = "sobel", threshold: int = 50
) -> np.ndarray:
    ...
    if method == "sobel":
        ...
        raise ValueError(
            "Unsupported edge detection method. Supported methods: sobel, canny"
        )
...
def compute_gradints(
    ...
    edge_down[down] = (
        edges[x_indices[down], y_indices[down]]
        != edges[x_indices[down] - 1, y_indices[down]]
    )

    left = np.where(y_indices > 0)
    edge_left[left] = (
        edges[x_indices[left], y_indices[left]]
        != edges[x_indices[left], y_indices[left] - 1]
    )

    right = np.where(y_indices < mask.shape[1] - 1)
    edge_right[right] = (
        edges[x_indices[right], y_indices[right]]
        != edges[x_indices[right], y_indices[right] + 1]
    )
